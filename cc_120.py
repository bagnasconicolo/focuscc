...
def update_feed():
    ...
    global cap, running, threshold, save_events, only_view, low_threshold, high_threshold, last_event_time, cooldown_enabled, contrast, brightness, saturation, black_point, detection_area
...
# Sixth column: Detection area controls
detection_area_frame = Frame(bottom_frame, bg="#f0f0f0", bd=2, relief="ridge")
detection_area_frame.grid(row=0, column=5, padx=5, pady=5, sticky="nsew")

Label(detection_area_frame, text="Detection Area X1", font=("Helvetica", 12), bg="#f0f0f0").pack(pady=5)
detection_area_x1_slider = Scale(detection_area_frame, from_=0, to=FRAME_WIDTH, orient="horizontal", command=update_detection_area_x1, bg="#e0e0e0", highlightthickness=0)
detection_area_x1_slider.set(detection_area[0])
detection_area_x1_slider.pack()

Label(detection_area_frame, text="Detection Area Y1", font=("Helvetica", 12), bg="#f0f0f0").pack(pady=5)
detection_area_y1_slider = Scale(detection_area_frame, from_=0, to=FRAME_HEIGHT, orient="horizontal", command=update_detection_area_y1, bg="#e0e0e0", highlightthickness=0)
detection_area_y1_slider.set(detection_area[1])
detection_area_y1_slider.pack()

Label(detection_area_frame, text="Detection Area X2", font=("Helvetica", 12), bg="#f0f0f0").pack(pady=5)
detection_area_x2_slider = Scale(detection_area_frame, from_=0, to=FRAME_WIDTH, orient="horizontal", command=update_detection_area_x2, bg="#e0e0e0", highlightthickness=0)
detection_area_x2_slider.set(detection_area[2])
detection_area_x2_slider.pack()

Label(detection_area_frame, text="Detection Area Y2", font=("Helvetica", 12), bg="#f0f0f0").pack(pady=5)
detection_area_y2_slider = Scale(detection_area_frame, from_=0, to=FRAME_HEIGHT, orient="horizontal", command=update_detection_area_y2, bg="#e0e0e0", highlightthickness=0)
detection_area_y2_slider.set(detection_area[3])
detection_area_y2_slider.pack()

# Preset controls
preset_frame = Frame(bottom_frame, bg="#f0f0f0", bd=2, relief="ridge")
preset_frame.grid(row=0, column=6, padx=5, pady=5, sticky="nsew")
